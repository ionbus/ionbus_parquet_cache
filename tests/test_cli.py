...
class TestUpdateCacheMain:
    ...
    def test_backfill_restate_mutually_exclusive(self, temp_cache: Path) -> None:
        """Should fail when --backfill and --restate both specified."""
        result = update_cache_main([
            str(temp_cache),
            "--backfill",
...
class TestSyncCacheMain:
    ...
    def test_s3_not_implemented(self, temp_cache: Path, tmp_path: Path) -> None:
...
class TestSyncPushPull:
    ...
    def test_push_syncs_dpd(self, configured_cache: Path, tmp_path: Path) -> None:
    ...
    def test_dry_run_no_copy(self, configured_cache: Path, tmp_path: Path) -> None:
...
class TestTrimUndoScript:
    ...
    def test_undo_script_deletes_new_metadata(self, tmp_path: Path) -> None:
        ...
        trimmed_files = [
            cache_path / "dataset" / "_meta_data" / "dataset_old_trimmed.pkl.gz",
            cache_path / "dataset" / "year=Y2024" / "data_old_trimmed.parquet",
        ]
        new_meta_files = [
...
class TestTrimmedFileFiltering:
    ...
    def test_discover_datasets_ignores_trimmed_metadata(
        ...
        real_suffix = real_meta_files[0].name.split("_")[-1].replace(".pkl.gz", "")
...
class TestFindOrphansScriptGeneration:
    ...
    def test_find_orphans_generates_script(self, configured_cache: Path) -> None:
        ...
        assert len(scripts) >= 1, "No cleanup script was generated for orphaned files"
