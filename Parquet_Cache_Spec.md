...
The suffix (e.g., `1Gz5hK`) is generated by
`ionbus_utils.general.timestamped_id(use_seconds=True)`: a base-62
encoded count of seconds since epoch, using digits and letters
(`0-9A-Za-z`). The suffix is exactly 6 characters (through approximately
year 3770 - 62^6 seconds from Unix epoch), ensuring lexicographic ordering
matches chronological ordering. Because the same suffix is applied to both
the new data files and the metadata pickle written in the same update run,
the pickle filename alone identifies all files produced in that run.
...
Both DPDs and NPDs use suffix-based versioning. The suffix (e.g., `1Gz5hK`)
is a base-62 encoded timestamp ensuring lexicographic ordering matches
...
- Assigns a new snapshot suffix (base-62 timestamp)
...
`{dataset_name}_{suffix}.pkl.gz`, where `suffix` is a base-62 encoded
timestamp generated by
`ionbus_utils.general.timestamped_id(use_seconds=True)` at the start of
each update run. The same suffix is used for all new parquet data files
and the metadata pickle written during that run. Each pickle file is a
complete **snapshot** of the dataset state at the time it was written.
...
#
# Note: Script names use a 6-character base-62 timestamp suffix (e.g., _cleanup_1H4Dw0.sh)
...
The cleanup script is written to the cache root with a base-62 timestamp suffix:
- **Linux/Mac:** `_cleanup_{suffix}.sh` (uses `rm` commands)
- **Windows:** `_cleanup_{suffix}.bat` (uses `del` commands)

The `{suffix}` is a 6-character base-62 encoded timestamp (e.g., `_cleanup_1H4Dw0.sh`).
...
3. **`NonDatedParquetDataset` uses snapshot imports.** Unlike DPDs which
   ...
   versioned snapshot (file or directory with base-62 suffix).
